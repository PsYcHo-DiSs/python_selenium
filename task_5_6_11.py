...
with webdriver.Chrome() as webdriver:
    webdriver.get(URL)
    for line in cookies:
        webdriver.add_cookie(line)
        webdriver.refresh()
        personal_dct = dict()
        cookie = webdriver.get_cookies()

        personal_dct.update(line)
        age = webdriver.find_element(By.ID, "age")
        personal_dct.setdefault("age", int(age.text.split("Age: ")[1]))

        ul_element = webdriver.find_element(By.ID, "skillsList")
        li_elements = ul_element.find_elements(By.TAG_NAME, "li")
        language_qty = len(li_elements)
        personal_dct.setdefault("skills", language_qty)

        personal_info.append(personal_dct)
        webdriver.delete_all_cookies()

sorted_data = sorted(personal_info, key=lambda x: (x["age"], -x["skills"]))
print(sorted_data[0]['value'])

#
# with open("cookie_miner.txt", "w") as file:
#     print(personal_info, file=file)
